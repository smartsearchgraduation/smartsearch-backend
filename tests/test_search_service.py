...
class TestSearchService:
    ...
    def test_execute_rawtext_search(self, mock_db, mock_faiss, mock_models):
        with patch('services.search_service.SearchQuery') as MQS, \
            ...
            mock_db.session.get.return_value = orig
    ...
    def test_get_search_by_id(self, mock_db):
        with patch('services.search_service.SearchQuery') as MQS:
            ...
            mock_db.session.get.return_value = sq
    ...
    def test_update_client_metrics(self, mock_db):
        with patch('services.search_service.SearchTime') as MST:
            st = MagicMock(); mock_db.session.get.return_value = st
    ...
    def test_execute_image_only_search(self, mock_db, mock_corrector, mock_faiss, mock_models):
        with patch('services.search_service.SearchQuery') as MQS, \
             patch('services.search_service.Retrieve'), \
             patch('services.search_service.SearchTime'):
            sq = MagicMock(); sq.search_id = 801; MQS.return_value = sq
            mock_faiss.search_image.return_value = {'products':[{'product_id':7,'score':0.9}],'success':True}
            r = SearchService.execute_search("", image="/t.jpg")
            assert r == {'search_id': 801}

    def test_execute_early_fusion(self, mock_db, mock_corrector, mock_faiss, mock_models):
        with patch('services.search_service.SearchQuery') as MQS, \
             patch('services.search_service.Retrieve'), \
             patch('services.search_service.SearchTime'):
            mock_models.return_value = {'fusion_endpoint':'early','textual_model':'m','visual_model':'m','fused_model':'m'}
            sq = MagicMock(); sq.search_id = 802; MQS.return_value = sq
            with patch('services.search_service.convert_to_jpg', return_value='/t.jpg'):
                mock_faiss.search_early_fusion.return_value = {'products':[{'product_id':8,'score':0.85}],'success':True}
                r = SearchService.execute_search("text", image="/t.png")
                assert r == {'search_id': 802}
    ...
    def test_db_fallback_search(self, mock_db):
        with patch('services.search_service.SearchQuery') as MQS:
            sq = MagicMock(); sq.raw_text = "test phone"; mock_db.session.get.return_value = sq
