...
class SearchService:
    ...
    def execute_search(
        ...
        search_mode = (search_mode or "std").strip().lower()
        if search_mode not in {"std", "iwt", "twi"}:
            raise ValueError(
                "Invalid search_mode. Allowed values: 'std', 'iwt', 'twi'"
            )
        logger.info(
            f"[Search] 🔘 Toggles: semantic={semantic_search_enabled}, correction={correction_enabled}, mode={search_mode}"
        )
        ...
        try:
            ...
            if correction_enabled:
                ...
                correction_result = text_corrector_service.correct(
                ...
                logger.info(
                    f"[Search] ℹ️  Skipping spell correction (correction_enabled=false)"
                )
            ...
            configured_fusion = selected_models.get(
                "fusion_endpoint", "late"
            )  # 'late' or 'early'
            configured_textual_model = selected_models.get(
            ...
            if semantic_search_enabled:
                ...
                logger.info(
                    f"[Search] ⚙️  Configured Fusion Endpoint: {configured_fusion}"
                )

                # Determine search type based on input
                has_text = bool(corrected_text and corrected_text.strip())
                has_image = bool(image)

                if search_mode == "iwt":
                    if not has_text:
                        raise ValueError("search_mode 'iwt' requires 'raw_text'")

                    fusion_type_actual = "cross_modal_iwt"
                    ...
                    if not has_image:
                        raise ValueError("search_mode 'twi' requires 'image'")

                    fusion_type_actual = "cross_modal_twi"
                ...
                # Parse FAISS results
                faiss_products = (
                    faiss_result.get("products") or faiss_result.get("results") or []
                )
                is_success = (
                    faiss_result.get("success") is True
                ...
                logger.info(
                    f"[Search] 🔍 Mode: Database ILIKE Search (Semantic Disabled)"
                )
                faiss_success = False
                faiss_result = {}
                faiss_products = []
            ...
            if semantic_search_enabled:
                logger.info(
                    f"[Search] 📊 FAISS Response Keys: {list(faiss_result.keys())}"
                )
                logger.info(
                    f"[Search] ✅ Status: {faiss_result.get('status', faiss_result.get('success', 'unknown'))}"
                )
            ...
            if semantic_search_enabled and faiss_success:
                ...
                for p in faiss_products:
                    raw_score = p.get("score", 1.0)
                    combined_score = p.get("combined_score")
                    # Early fusion: use score as combined_score if not present
                    if combined_score is None and raw_score is not None:
                        combined_score = raw_score
                    # Use combined_score as the primary score for both early and late fusion
                    score = combined_score if combined_score is not None else raw_score
                    product_data = {
                ...
                if search_mode == "std" and image and corrected_text and corrected_text.strip():
                    if (
                        products
                        and products[0].get("text_score") is not None
                        and products[0].get("image_score") is not None
                    ):
                        fusion_type_actual = "late_fusion"
                    elif products and (
                        products[0].get("combined_score") is not None
                        or products[0].get("score") is not None
                    ):
                        # Early fusion returns score (combined embedding),
                        # may also have combined_score field
                        fusion_type_actual = "early_fusion"
                    else:
                        # Fallback to configured value if set, else late_fusion
                        if not fusion_type_actual:
                            fusion_type_actual = "late_fusion"
                ...
                for i, p in enumerate(products[:5], 1):
                    if fusion_type_actual == "late_fusion":
                        logger.info(
                            f"[Search]    {i}. product_id={p['product_id']}, combined={(p.get('combined_score') or p.get('score') or 0):.6f}, text={(p.get('text_score') or 0):.6f}, image={(p.get('image_score') or 0):.6f}"
                        )
                    else:
                        logger.info(
                            f"[Search]    {i}. product_id={p['product_id']}, score={(p.get('score') or 0):.6f}"
                        )
            else:
                # FAISS failed/empty -> Return empty results (no DB fallback)
                logger.info(
                    f"[Search] ⚠️  FAISS returned empty, no DB fallback (semantic={semantic_search_enabled})"
                )
                products = []
            ...
            logger.info(
                f"[Search] 💾 Saving {len(products)} retrieve records... (fusion_type: {fusion_type_actual})"
            )
            ...
            return {"search_id": search_query.search_id}

        except Exception as e:
            db.session.rollback()
            total_duration = (time.time() - start_total) * 1000
            logger.error(
                f"[Search] ❌ Error executing search: {e} (failed after {total_duration:.2f}ms)"
            )
            raise
    ...
    def execute_rawtext_search(
        ...
        logger.info(f"[Search] 🖼️  Image: {image if image else 'None'}")
        logger.info(
            f"[Search] 🔘 Toggles: semantic={semantic_search_enabled}, correction={correction_enabled}"
        )
        ...
        try:
            ...
            logger.info(
                f"[Search] 🔄 Updating original search (ID: {original_search_id}) links..."
            )

            # Update original search's retrieve records to mark rawtext_used and link new_search_id
            original_retrieves = Retrieve.query.filter_by(
            ...
            total_duration = (time.time() - start_total) * 1000
            logger.error(
                f"[Search] ❌ Error executing raw text search: {e} (failed after {total_duration:.2f}ms)"
            )
            raise
        ...
        try:
            ...
            for row in rows:
                ...
                for img_path in image_paths:
                    ...
                    if os.path.exists(file_path):
                        try:
                            ...
                            logger.warning(
                                f"[Search] Failed to read image {file_path}: {e}"
                            )
                ...
                product_data = {
                    ...
                    "rank": row[2],
                    "score": float(row[3]) if row[3] else None,
                    "brand": {"brand_id": row[14], "name": row[13]}
                    if row[13]
                    else None,
    ...
    def update_client_metrics(
        search_id: int, search_duration: float, product_load_duration: float
    ) -> bool:
        """
        ...
        try:
            ...
            if not search_time:
                ...
                logger.warning(
                    f"[Analytics] SearchTime not found for search_id={search_id}"
                )
                return False
    ...
    def execute_db_fallback_search(search_id: int) -> Dict[str, Any]:
        ...
        performs a simple DB ILIKE search, and returns all matching results.
        ...
        Returns:
            A dict with 'products' containing all matching results
        """
        start_total = time.time()
        ...
        try:
            ...
            db_products = (
                Product.query.filter(and_(*and_conditions))
                ...
                for img in p.images:
                    ...
                    if os.path.exists(file_path):
                        try:
                            ...
                            logger.warning(
                                f"[DB Fallback] Failed to read image {file_path}: {e}"
                            )
                ...
                products.append(
                    {
                        ...
                        "score": 1.0,
                        "brand": {"brand_id": p.brand.brand_id, "name": p.brand.name}
                        if p.brand
                        else None,
                        "images": images_base64,
                    }
            ...
            logger.info(
                f"[Search] 📂 DB returned {len(products)} products (took {search_duration:.2f}ms)"
            )
            ...
            total_duration = (time.time() - start_total) * 1000
            logger.error(
                f"[Search] ❌ Error executing DB fallback search: {e} (failed after {total_duration:.2f}ms)"
            )
            raise
    ...
    def get_metrics() -> Dict[str, Any]:
        ...
        avg_time = (
            db.session.query(db.func.avg(SearchQuery.time_to_retrieve)).scalar() or 0
        )

        # Also get avg backend total time from new table
        avg_backend = (
            db.session.query(db.func.avg(SearchTime.backend_total_time)).scalar() or 0
        )
