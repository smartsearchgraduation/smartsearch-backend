...
FAISS_SERVICE_URL = os.getenv(
    "FAISS_SERVICE_URL", "http://localhost:5002/api/retrieval/search"
)
FAISS_ADD_PRODUCT_URL = os.getenv(
    "FAISS_ADD_PRODUCT_URL", "http://localhost:5002/api/retrieval/add-product"
)
FAISS_UPDATE_PRODUCT_URL = os.getenv(
    "FAISS_UPDATE_PRODUCT_URL", "http://localhost:5002/api/retrieval/update-product"
)
FAISS_DELETE_PRODUCT_URL = os.getenv(
    "FAISS_DELETE_PRODUCT_URL", "http://localhost:5002/api/retrieval/delete-product"
)
FAISS_TEXT_SEARCH_URL = os.getenv(
    "FAISS_TEXT_SEARCH_URL", "http://localhost:5002/api/retrieval/search/text"
)
FAISS_IMAGE_SEARCH_URL = os.getenv(
    "FAISS_IMAGE_SEARCH_URL", "http://localhost:5002/api/retrieval/search/image"
)
FAISS_LATE_FUSION_URL = os.getenv(
    "FAISS_LATE_FUSION_URL", "http://localhost:5002/api/retrieval/search/late"
)
FAISS_EARLY_FUSION_URL = os.getenv(
    "FAISS_EARLY_FUSION_URL", "http://localhost:5002/api/retrieval/search/early"
)
FAISS_IMAGE_BY_TEXT_URL = os.getenv(
    "FAISS_IMAGE_BY_TEXT_URL",
    "http://localhost:5002/api/retrieval/search/image-by-text",
)
FAISS_TEXT_BY_IMAGE_URL = os.getenv(
    "FAISS_TEXT_BY_IMAGE_URL",
    "http://localhost:5002/api/retrieval/search/text-by-image",
)
FAISS_MODELS_URL = os.getenv(
    "FAISS_MODELS_URL", "http://localhost:5002/api/retrieval/models"
)
FAISS_INDEX_STATS_URL = os.getenv(
    "FAISS_INDEX_STATS_URL", "http://localhost:5002/api/retrieval/index-stats"
)
FAISS_HEALTH_URL = os.getenv("FAISS_HEALTH_URL", "http://localhost:5002/api/health")
FAISS_CLEAR_INDEX_URL = os.getenv(
    "FAISS_CLEAR_INDEX_URL", "http://localhost:5002/api/retrieval/clear-index"
)
...
class FAISSRetrievalService:
    ...
    def search(self, query_text: str, top_k: int = 10) -> Dict[str, Any]:
        """
        ...
        if not HAS_REQUESTS:
            logger.warning(
                "[FAISS] requests library not installed, returning empty results"
            )
        ...
        try:
            logger.info(
                f"[FAISS] Calling service at {self.base_url} with query: '{query_text}'"
            )
            response = requests.post(
                self.base_url, json={"query": query_text, "top_k": top_k}, timeout=10
            )
            ...
            if not products:
                logger.info(
                    f"[FAISS] Query: '{query_text}' -> empty results (took {duration:.2f}ms)"
                )
                return {"products": [], "success": False}

            logger.info(
                f"[FAISS] Query: '{query_text}' -> {len(products)} results (took {duration:.2f}ms)"
            )
            ...
            duration = (time.time() - start_time) * 1000
            logger.warning(
                f"[FAISS] Service not available at {self.base_url} (took {duration:.2f}ms)"
            )
            return {"products": [], "success": False}
    ...
    def search_text(
        ...
        try:
            ...
            response = requests.post(self.text_search_url, json=payload, timeout=10)
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"[FAISS] Text search failed: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
    ...
    def search_image(
        self, image_path: str, visual_model_name: str = "ViT-B/32", top_k: int = 10
    ) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"[FAISS] Image search: '{image_path}' (model={visual_model_name})"
            )
            payload = {
                "image": image_path,
            ...
            response = requests.post(self.image_search_url, json=payload, timeout=15)
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"[FAISS] Image search failed: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
    ...
    def search_early_fusion(
        ...
        try:
            ...
            response = requests.post(self.early_fusion_url, json=payload, timeout=15)
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"[FAISS] Early fusion search failed: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
    ...
    def search_image_by_text(
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"[FAISS] Image-by-text search failed: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
    ...
    def search_text_by_image(
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"[FAISS] Text-by-image search failed: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
    ...
    def search_late_fusion(
        ...
        try:
            ...
            response = requests.post(self.late_fusion_url, json=payload, timeout=15)
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"[FAISS] Late fusion search failed: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
    ...
    def add_product(
        ...
        try:
            logger.info(
                f"[FAISS] Adding product {product_id} with {len(images)} images"
            )
            ...
            logger.info(f"[FAISS] Upload folder from config: {upload_folder}")

            # Check that image files exist
            valid_images = []
            if images and isinstance(images, list):
                logger.info(
                    f"[FAISS] Received {len(images)} image paths for validation"
                )
                for idx, img_path in enumerate(images):
                    logger.info(f"[FAISS]   Checking image {idx}: raw={img_path}")

                    # Handle different path formats
                    if img_path.startswith("/uploads/products/") or img_path.startswith(
                        "/"
                    ):
                        # Web URL path - convert to file system path
                        # Remove leading slash and join with project root
                        project_root = os.path.dirname(current_app.root_path)
                        relative_path = img_path.lstrip("/")
                        full_path = os.path.join(project_root, relative_path)
                        logger.info(f"[FAISS]   Web path converted: {full_path}")
                    elif not os.path.isabs(img_path):
                        full_path = os.path.join(upload_folder, img_path)
                        logger.info(f"[FAISS]   Relative path joined: {full_path}")
                    else:
                        full_path = img_path
                        logger.info(f"[FAISS]   Absolute path: {full_path}")

                    # Normalize path
                    norm_path = os.path.normpath(full_path)
                    exists = os.path.exists(norm_path)

                    logger.info(f"[FAISS]   Normalized: {norm_path}, exists={exists}")

                    if not exists:
                        logger.warning(f"[FAISS]   ✗ Image not found: {norm_path}")
                    else:
                        valid_images.append(norm_path)
                        logger.info(f"[FAISS]   ✓ Valid image: {norm_path}")
            else:
                logger.warning(
                    f"[FAISS] No images received or invalid format: {images}"
                )

            logger.info(
                f"[FAISS] Total valid images: {len(valid_images)}/{len(images) if images else 0}"
            )
            ...
            response = requests.post(self.add_product_url, json=payload, timeout=120)
            ...
            if response.status_code not in [200, 201]:
                ...
                logger.error(
                    f"[FAISS] Failed to add product {product_id}: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
            ...
            # Check if the product was skipped (already exists)
            if result.get("status") == "success" and result.get("details", {}).get(
                "skipped"
            ):
                logger.warning(
                    f"[FAISS] Product {product_id} skipped - already exists in index"
                )
                return {
                    "status": "skipped",
                    "message": f"Product {product_id} already has embeddings for this model",
                    "details": result.get("details", {}),
                }

            if result.get("success", False) or result.get("status") == "success":
                data = result.get("data", result.get("details", {}))
                textual_vector_id = data.get("textual_vector_id")
                visual_vector_ids = data.get("visual_vector_ids", [])
                images_processed = len(visual_vector_ids)

                # Validate that we actually got vector IDs
                if textual_vector_id is None:
                    logger.error(
                        f"[FAISS] Product {product_id} returned success but no textual_vector_id"
                    )
                    return {
                        "status": "error",
                        "error": "FAISS returned success but no textual vector ID was assigned",
                    }
    ...
    def update_product(
        ...
        try:
            logger.info(
                f"[FAISS] Updating product {product_id} with {len(images)} images"
            )
            ...
            if response.status_code not in [200, 201]:
                ...
                logger.error(
                    f"[FAISS] Failed to update product {product_id}: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
    ...
    def delete_product(self, product_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 404:
                logger.warning(
                    f"[FAISS] Product {product_id} not found in FAISS index (took {duration:.2f}ms)"
                )
                return {
                    "status": "success",
                    "message": f"Product {product_id} not in FAISS index",
                }
            ...
            if response.status_code not in [200, 204]:
                ...
                logger.error(
                    f"[FAISS] Failed to delete product {product_id}: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
            ...
            logger.info(
                f"[FAISS] Successfully deleted product {product_id} from index (took {duration:.2f}ms)"
            )
    ...
    def get_available_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"[FAISS] Models endpoint returned {response.status_code}, using local config"
                )
                return self._get_local_models()

        except requests.exceptions.ConnectionError:
            logger.warning(
                f"[FAISS] Service not available at {self.models_url}, using local config"
            )
            return self._get_local_models()
        except requests.exceptions.Timeout:
            logger.warning(
                f"[FAISS] Request timeout fetching models, using local config"
            )
            return self._get_local_models()
        except Exception as e:
    ...
    def get_index_stats(self) -> Dict[str, Any]:
        ...
        try:
            logger.info(
                f"[FAISS] Fetching index statistics from {self.index_stats_url}"
            )

            response = requests.get(self.index_stats_url, timeout=10)
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    f"[FAISS] Index stats endpoint returned {response.status_code}"
                )
                return {
                    "status": "error",
    ...
    def _get_local_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            textual_models = [
                get_model_info(model_id) for model_id in AVAILABLE_MODELS.keys()
            ]
            ...
            return {"status": "error", "error": f"Failed to load models: {str(e)}"}
    ...
    def clear_index(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 404:
                logger.warning(
                    f"[FAISS] Clear index endpoint not found (took {duration:.2f}ms)"
                )
                return {
                    "status": "success",
            ...
            if response.status_code not in [200, 204]:
                ...
                logger.error(
                    f"[FAISS] Failed to clear index: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
            ...
            return {
                ...
                "details": {"deleted_count": deleted_count, "duration_ms": duration},
            }
    ...
    def save_selected_models(
        ...
        try:
            logger.info(
                f"[FAISS] Saving selected models - Textual: {textual_model}, Visual: {visual_model}"
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"[FAISS] Save models failed: {response.status_code} - {error_msg}"
                )
                return {
                    "status": "error",
                    "error": f"FAISS Error ({response.status_code}): {error_msg}",
