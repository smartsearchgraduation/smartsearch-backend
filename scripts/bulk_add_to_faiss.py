...
UPLOAD_FOLDER = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "uploads", "products"
)
# ============================================
...
def add_product_to_faiss(product_data: dict) -> dict:
    ...
    try:
        response = requests.post(FAISS_ADD_PRODUCT_URL, json=product_data, timeout=120)
        return response.json()
    except requests.exceptions.ConnectionError:
        return {
            "status": "error",
            "error": "FAISS service not available at " + FAISS_ADD_PRODUCT_URL,
        }
...
def main():
    ...
    with app.app_context():
        # Get all active products with eager loading for brand and images
        products = (
